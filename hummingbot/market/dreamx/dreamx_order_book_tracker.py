...
from hummingbot.market.idex.dreamx_order_book import DREAMXOrderBook
from hummingbot.market.idex.dreamx_active_order_tracker import DREAMXActiveOrderTracker
from hummingbot.market.idex.dreamx_api_order_book_data_source import DREAMXAPIOrderBookDataSource
from hummingbot.market.idex.dreamx_order_book_message import DREAMXOrderBookMessage
from hummingbot.market.idex.dreamx_order_book_tracker_entry import DREAMXOrderBookTrackerEntry
...
class DREAMXOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
        ...
        self._order_books: Dict[str, DREAMXOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[DREAMXOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._order_book_diff_stream: asyncio.Queue = asyncio.Queue()
        self._order_book_snapshot_stream: asyncio.Queue = asyncio.Queue()
        self._ev_loop: asyncio.BaseEventLoop = asyncio.get_event_loop()
        self._data_source: Optional[OrderBookTrackerDataSource] = None
        self._active_order_trackers: Dict[str, DREAMXActiveOrderTracker] = defaultdict(DREAMXActiveOrderTracker)
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.EXCHANGE_API:
                self._data_source = DREAMXAPIOrderBookDataSource(trading_pairs=self._trading_pairs)
            else:
    ...
    async def _refresh_tracking_tasks(self):
        ...
        available_pairs: Dict[str, DREAMXOrderBookTrackerEntry] = await self.data_source.get_tracking_pairs()
        ...
        for trading_pair in new_trading_pairs:
            order_book_tracker_entry: DREAMXOrderBookTrackerEntry = available_pairs[trading_pair]
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: DREAMXOrderBookMessage = await self._order_book_diff_stream.get()
                ...
                order_book: DREAMXOrderBook = self._order_books[trading_pair]
                ...
                if ob_message.content["event"] == "MarketTrades":  # put trade messages to trade queue
                    trade_type = float(TradeType.SELL.value) if ob_message.content["sell"] \
                        else float(TradeType.BUY.value)
                    self._order_book_trade_stream.put_nowait(OrderBookMessage(OrderBookMessageType.TRADE, {
                        "trading_pair": ob_message.content["market_symbol"],
                        "trade_type": trade_type,
                        "trade_id": ob_message.content["id"],
                        "update_id": ob_message.timestamp,
                        "price": ob_message.content["price"],
                        "amount": ob_message.content["give_amount"]
    ...
    async def _track_single_book(self, trading_pair: str):
        past_diffs_window: Deque[DREAMXOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window
        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: DREAMXOrderBook = self._order_books[trading_pair]
        active_order_tracker: DREAMXActiveOrderTracker = self._active_order_trackers[trading_pair]
        last_message_timestamp: float = time.time()
        diff_messages_accepted: int = 0

        while True:
            try:
                message: DREAMXOrderBookMessage = None
                saved_messages: Deque[DREAMXOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[DREAMXOrderBookMessage] = list(past_diffs_window)
